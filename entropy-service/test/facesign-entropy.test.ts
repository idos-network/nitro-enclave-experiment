...
describe("FaceSign Wallet Entropy API", () => {
	...
	it("new user no entropy", async () => {
		...
		expect(agentSpy).toHaveBeenCalledWith("facesign-wallet-entropy-created", {
			userId,
			ip: "::ffff:127.0.0.1",
		});
	});
	...
	it("user with existing entropy", async () => {
		...
		expect(entropySpy).toHaveBeenCalledWith(userId);
		expect(agentSpy).toHaveBeenCalledWith("facesign-wallet-entropy-fetched", {
			userId,
			ip: "::ffff:127.0.0.1",
		});
	});
	...
	it("expired token", async () => {
		...
		expect(agentSpy).toHaveBeenCalledWith("facesign-wallet-entropy-error-iat", {
