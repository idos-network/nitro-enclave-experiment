...
describe("Entropy API", () => {
  it("missing token", async () => {
    const response = await request(app).post("/entropy").send({});
  ...
  it("new user no entropy", async () => {
    ...
    const entropySpy = vi.spyOn(db, "fetchOrCreateEntropy").mockResolvedValue({
      insert: true,
    ...
    expect(agentSpy).toHaveBeenCalledWith("new-entropy-created", { userId, ip: "::ffff:127.0.0.1" });
  });
  ...
  it("user with existing entropy", async () => {
    ...
    expect(entropySpy).toHaveBeenCalledWith(userId);
    expect(agentSpy).toHaveBeenCalledWith("existing-entropy-fetched", { userId, ip: "::ffff:127.0.0.1" });
  });
  ...
  it("invalid token", async () => {
    ...
    expect(agentSpy).toHaveBeenCalledWith("error-verify", {
  ...
  it("expired token", async () => {
    ...
    expect(agentSpy).toHaveBeenCalledWith("error-iat", {
