...
app.post("/pinocchio-entropy", async (req, res) => {
  ...
  try {
    ...
    agent.writeLog("pinocchio-error-verify", { message: "Invalid token", error });
    return res.status(400).json({ error: "Invalid token" });
  }

  if (!result.iat || !result.sub) {
    agent.writeLog("pinocchio-error-validate", { message: "Token missing iat or sub" });
    return res.status(400).json({ error: "Invalid token" });
  }
  ...
  const { insert, entropy } = await fetchOrCreatePinocchioEntropy(result.sub as string);

  agent.writeLog(
    `pinocchio-entropy-${insert ? "created" : "fetched"}`,
    { userId: result.sub, ip: req.ip }
  );
