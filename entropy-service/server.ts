...
import { fetchOrCreateFaceSignWalletEntropy } from "./providers/db.ts";
...
app.post("/facesign-wallet/entropy", async (req, res) => {
	...
	try {
		...
		agent.writeLog("facesign-wallet-entropy-error-verify", {
	...
	if (!result.iat || !result.sub) {
		agent.writeLog("facesign-wallet-entropy-error-validate", {
	...
	if (Date.now() / 1000 - result.iat > 1 * 60) {
		agent.writeLog("facesign-wallet-entropy-error-iat", {
	...
	const { insert, entropy } = await fetchOrCreateFaceSignWalletEntropy(
		result.sub as string,
	);

	agent.writeLog(`facesign-wallet-entropy-${insert ? "created" : "fetched"}`, {
		userId: result.sub,
		ip: req.ip,
	});
