...
export const login = async (req: Request, res: Response) => {
  ...
  if (!success || !result.livenessProven || didError) {
    agent.writeLog("facesign-wallet-enrollment-failed", { success, result, didError });
  ...
  const { newUser, entropyToken, confirmationToken, faceSignUserId } = await faceSignWalletLogin(
    generatedUserId,
    false,
  );
...
export const confirmation = async (req: Request, res: Response) => {
  ...
  agent.writeLog("facesign-wallet-user-confirmed", {
