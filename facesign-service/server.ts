...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (searchResult.success) {
      ...
      console.log(
        "Group does not exist, creating one by enrolling first user.",
      );
      results = [];
    } else {
      throw new Error(
        "Failed to search for duplicates, check application logs.",
      );
    }
...
app.get("/idos/keys/1", (_req, res) => {
  const publicKeyMultibase = fs
    .readFileSync(KEY_1_MULTIBASE_PUBLIC_PATH, "utf-8")
    .trim();
