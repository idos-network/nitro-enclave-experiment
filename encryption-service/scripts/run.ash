...
aws s3 cp "s3://$S3_SECRETS_BUCKET/encryption/vsock.json" ./vsock.json --region eu-west-1
source "$SCRIPT_DIR/shared/vsock.ash"
setup_vsock_networking "./vsock.json"

# Get config.env and source it
aws s3 cp "s3://$S3_SECRETS_BUCKET/encryption/config.env" ./config.env --region eu-west-1
...
setup_nbd "encryption"

echo "Fetching Caddyfile from S3"
CADDYFILE=Caddyfile
aws s3 cp "s3://$S3_SECRETS_BUCKET/encryption/$CADDYFILE" "./$CADDYFILE" --region eu-west-1
if [ ! -f "./$CADDYFILE" ]; then
